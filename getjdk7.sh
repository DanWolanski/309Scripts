...
LATESTJDK=$(curl -s http://www.oracle.com/technetwork/java/javase/downloads/java-archive-downloads-javase7-521261.html | grep -oP 'http://download.oracle.com/otn/java/jdk/7u.*?/jdk-7u.*?-linux-x64.tar.gz' | head -1)
fi
next
...
FILENAME=$(echo $LATESTJDK  | grep -oP "jdk-.u.*?-linux-x64.tar.gz")
fi
